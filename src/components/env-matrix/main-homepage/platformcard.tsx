...
//     <div
//       onClick={() => onSelect(platform.type)}
//       className={`relative group ${theme.card} border-2 rounded-2xl p-6 cursor-pointer transition-all duration-300 hover:-translate-y-1 hover:shadow-lg overflow-hidden`}
//     >
//       <div className={`absolute -top-20 -right-20 w-64 h-64 ${
//         isDark 
//           ? 'bg-gradient-to-br from-teal-900/20 to-transparent'
...
//                 ? isDark ? 'bg-orange-600 hover:bg-orange-700' : 'bg-orange-600 hover:bg-orange-700'
//                 : theme.accent
//             } text-white rounded-lg font-semibold text-sm shadow-lg transition-all hover:scale-105`}
//           >
//             Enter Platform →
//           </button>
//         </div>
...
export default function PlatformCard({ platform, onSelect, theme, isDark }: PlatformCardProps) {
  ...
  return (
    <div
      onClick={() => onSelect(platform.type)}
      className={`relative group ${theme.card} border-2 rounded-xl p-5 cursor-pointer transition-all duration-300 hover:-translate-y-1 hover:shadow-lg overflow-hidden`}
    >
      ...
      <div className="relative z-10">
        ...
        <div className="flex items-start justify-between mb-3">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              {/* Title - Smaller */}
              <h3 className={`text-lg font-bold ${theme.text} mb-0.5`}>
                {platform.name}
              </h3>
              {/* Description - Smaller */}
          ...
          {/* Online Badge - Smaller */}
          <div className={`flex items-center gap-1.5 px-2 py-0.5 ${
            isDark ? 'bg-green-900/30' : 'bg-green-100'
          } rounded-full`}>
            <div className={`w-1.5 h-1.5 ${isDark ? 'bg-green-400' : 'bg-green-500'} rounded-full animate-pulse`}></div>
            <span className={`text-xs font-semibold ${isDark ? 'text-green-300' : 'text-green-700'}`}>
              Online
            </span>
          </div>
          ...
          <button
            className={`px-4 py-1.5 ${
              isNonHydra
                ? isDark ? 'bg-orange-600 hover:bg-orange-700' : 'bg-orange-600 hover:bg-orange-700'
                : theme.accent
            } text-white rounded-lg font-semibold text-xs shadow-lg transition-all hover:scale-105`}
          >
            Enter Platform →
          </button>
        </div>
